...
from qvevri.settings import RUNTIME_DIR, RUNTIME_URL
from qvevri.util import http, jobs, system
from qvevri.util.downloader import Downloader
from qvevri.util.extract import extract_archive
from qvevri.util.log import logger
from qvevri.util.system import LINUX_SYSTEM
...
def get_env(version=None, prefer_system_libs=False, wine_path=None):
    ...
    Params:
        ...
        wine_path (str): If you prioritize system libs, provide the path for a qvevri wine build
                         if one is being used. This allows Qvevri to prioritize the wine libs
...
def get_winelib_paths(wine_path):
    """Return wine libraries path for a Qvevri wine build"""
    paths = []
    # Prioritize libwine.so.1 for qvevri builds
...
def get_runtime_paths(version=None, prefer_system_libs=True, wine_path=None):
    """Return Qvevri runtime paths"""
    version = version or DEFAULT_RUNTIME
    if version.startswith("Ubuntu"):
        qvevri_runtime_path = "%s-i686" % version
    elif version == "legacy":
        qvevri_runtime_path = "lib32"
    else:
    ...
    if system.LINUX_SYSTEM.is_64_bit:
        if version == "legacy":
            qvevri_runtime_path = "lib64"
        else:
            qvevri_runtime_path = "%s-x86_64" % version
        runtime_paths += [
            qvevri_runtime_path,
