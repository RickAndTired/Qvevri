...
from qvevri.gui.dialogs import WineNotInstalledWarning
from qvevri.util import system
from qvevri.util.display import DISPLAY_MANAGER
from qvevri.util.strings import unpack_dependencies
from qvevri.util.jobs import AsyncCall
from qvevri.util.log import logger
from qvevri.util.steam.log import get_app_state_log
from qvevri.util.http import Request, HTTPError
from qvevri.util.wine.wine import get_wine_version_exe, get_system_wine_version

from qvevri.config import QvevriConfig, make_game_config_id

from qvevri.installer.errors import ScriptingError, FileNotAvailable, MissingGameDependency
from qvevri.installer.commands import CommandsMixin
from qvevri.installer.installer_file import InstallerFile

from qvevri.services import UnavailableGame
from qvevri.services.gog import get_gog_download_links, MultipleInstallerError
from qvevri.services.humblebundle import get_humble_download_link

from qvevri.runners import (
    wine,
    winesteam,
    steam,
    import_runner,
...
class ScriptInterpreter(CommandsMixin):
    ...
    def get_default_target(self):
        ...
        config = QvevriConfig(runner_slug=self.runner)
        ...
        if self.files:
            if (
                ...
                try:
                    ...
                    raise ScriptingError(
                        "Qvevri does not have the necessary permissions to install to path:",
                        self.target_path,
                    )
    ...
    def install_steam_game(self, runner_class=None, is_game_files=False):
        ...
        if not steam_runner.get_game_path_from_appid(appid):
            ...
            steam_runner.config = QvevriConfig(runner_slug=steam_runner.name)
    ...
    def _download_steam_data(self, file_uri, file_id):
        ...
        file_id: The qvevri installer internal id for the game files
        """
        try:
