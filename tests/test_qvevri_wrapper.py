...
if os.path.isfile('share/qvevri/bin/qvevri-wrapper'):
    qvevri_wrapper_bin = 'share/qvevri/bin/qvevri-wrapper'
else:
    qvevri_wrapper_bin = 'qvevri-wrapper'


class QvevriWrapperTestCase(unittest.TestCase):
    def test_excluded_initial_process(self):
        "Test that an excluded process that starts a monitored process works"
        env = os.environ.copy()
        env['PYTHONPATH'] = ':'.join(sys.path)
        # run the qvevri-wrapper with a bash subshell. bash is "excluded"
        ...
        try:
            ...
            # Wait a short while to see if qvevri-wrapper will exit (it shouldn't)
            try:
    ...
    def test_cleanup_children(self):
        ...
        env = os.environ.copy()
        env['PYTHONPATH'] = ':'.join(sys.path)
        # First, we run the qvevri-wrapper with a bash subshell which ignores
        ...
        try:
            ...
            # Wait for confirmation that qvevri-wrapper got our signal.
            for line in wrapper_proc.stdout:
            ...
            # verify that qvevri-wrapper closes.
            wrapper_proc.wait(30)
        finally:
            if wrapper_proc.returncode is None:
                wrapper_proc.kill()
