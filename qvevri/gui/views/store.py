...
from qvevri.gui.widgets.utils import get_pixbuf_for_game
from qvevri.util.resources import get_icon_path, download_media, update_desktop_icons
...
from qvevri.gui.views.pga_game import PgaGame
from . import (
    COL_ID,
    COL_SLUG,
    COL_NAME,
...
class GameStore(GObject.Object):
    ...
    def get_missing_media(self, slugs=None):
        """Query the Qvevri.net API for missing icons"""
        ...
        qvevri_media = api.get_api_games(list(missing_media_slugs), inject_aliases=True)
        if not qvevri_media:
            return

        for game in qvevri_media:
