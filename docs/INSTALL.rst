...
Qvevri should work on any up to date Linux system. It is based on Python and
...
These dependencies are only for running the Qvevri client. To install and run
...
Qvevri uses Python's distutils framework for installation. In order to
install Qvevri, you will need root access. To install Qvevri, perform
...
version without you having to go through /usr/local and manually delete qvevri
related files. The setup.py script is used by packaging tools to build the
actual packages.

Prefer installing Qvevri through distribution packages or run it directly
...
If you installed Qvevri using a package or the setup.py script, you can launch
the program by typing "qvevri" at the command line. If you want to run Qvevri
without installing it, start "bin/qvevri" from within the Qvevri directory.
