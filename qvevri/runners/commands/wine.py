...
from qvevri.runners import import_runner
from qvevri.command import MonitoredCommand
from qvevri.util import system
from qvevri.util.strings import split_arguments
...
from qvevri.util.wine.cabinstall import CabInstaller
...
def set_regedit_file(filename, wine_path=None, prefix=None, arch=WINE_DEFAULT_ARCH):
    ...
    if arch == "win64" and wine_path and system.path_exists(wine_path + "64"):
        ...
        # a bug in Wine. see: https://github.com/qvevri/qvevri/issues/804
...
def create_prefix(
    ...
    if 'steamapps/common' in prefix.lower():
        from qvevri.runners.winesteam import winesteam
        runner = winesteam()
...
def wineexec(
    ...
    Args:
        ...
        config (QvevriConfig): QvevriConfig object for the process context
    ...
    wine_config = config or QvevriConfig(runner_slug="wine")
    disable_runtime = disable_runtime or wine_config.system_config["disable_runtime"]
    if use_qvevri_runtime(wine_path=wineenv["WINE"], force_disable=disable_runtime):
