...
from qvevri.config import QvevriConfig, make_game_config_id
from qvevri.util.log import logger
from qvevri import runners
from qvevri import settings
from qvevri.cache import get_cache_path, save_cache_path
from qvevri.gui.widgets.common import VBox, SlugEntry, NumberEntry, Label, FileChooserEntry
from qvevri.gui.config.boxes import GameBox, RunnerBox, SystemBox
from qvevri.gui.dialogs import ErrorDialog, QuestionDialog
from qvevri.gui.widgets.log_text_view import LogTextView
from qvevri.gui.widgets.utils import (
    get_pixbuf_for_game,
    get_pixbuf,
    BANNER_SIZE,
    ICON_SIZE,
)
from qvevri.util.strings import slugify
from qvevri.util import resources
from qvevri.util.linux import gather_system_info_str
...
class GameDialogCommon:
    ...
    def _build_prefs_tab(self):
        ...
        self._add_notebook_tab(info_sw, "Qvevri preferences")
...
# remove install runners button on install       runner_box.pack_start(install_runners_btn, True, True, 0)
    ...
    def _build_runner_tab(self, _config_level):
        if self.runner_name:
            self.runner_box = RunnerBox(self.qvevri_config, self.game)
    ...
    def _build_system_tab(self, _config_level):
        if not self.qvevri_config:
            raise RuntimeError("Qvevri config not loaded yet")
        self.system_box = SystemBox(self.qvevri_config)
    ...
    def is_valid(self):
        ...
        if (
                ...
                and self.qvevri_config.game_config.get("appid") is None
        ):
            ErrorDialog("Steam AppId not provided")
            return False
        invalid_fields = []
        runner_module = importlib.import_module("qvevri.runners." + self.runner_name)
        runner_class = getattr(runner_module, self.runner_name)
        runner_instance = runner_class()
        for config in ["game", "runner"]:
            for k, v in getattr(self.qvevri_config, config + "_config").items():
    ...
    def on_save(self, _button):
        ...
        runner = runner_class(self.qvevri_config)

        self.game.name = name
        self.game.slug = self.slug
        self.game.year = year
        self.game.game_config_id = self.qvevri_config.game_config_id
        self.game.runner = runner
        self.game.runner_name = self.runner_name
        self.game.directory = runner.game_path
        self.game.is_installed = True
        if self.runner_name in ("steam", "winesteam"):
            self.game.steamid = self.qvevri_config.game_config["appid"]

        self.game.config = self.qvevri_config
        self.game.save()
        self.destroy()
        self.saved = True
