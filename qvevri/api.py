"""Functions to interact with the Qvevri REST API"""
import os
import re
import json
...
from qvevri.util import http, system
...
def read_api_key():
    ...
    try:
        ...
        logger.error("Unable to read Qvevri token in %s", API_KEY_FILE_PATH)
        return None
    return {"token": token, "username": username}


def connect(username, password):
    """Connect to the Qvevri API"""
...
def get_game_api_page(game_ids, page="1", query_type="games"):
    ...
    Args:
        ...
        query_type (str): Type of the IDs in game_ids, by default 'games' queries
                          games by their Qvevri slug. 'gogid' can also be used.
    """
...
def get_api_games(game_slugs=None, page="1", query_type="games", inject_aliases=False):
    """Return all games from the Qvevri API matching the given game slugs"""
...
def parse_installer_url(url):
    """
    Parses `qvevri:` urls, extracting any info necessary to install or run a game.
    """
    action = None
    try:
        parsed_url = urllib.parse.urlparse(url, scheme="qvevri")
    except Exception:  # pylint: disable=broad-except
        logger.warning("Unable to parse url %s", url)
        return False
    if parsed_url.scheme != "qvevri":
        return False
    ...
    if len(url_parts) == 2:
        ...
        raise ValueError("Invalid qvevri url %s" % url)
