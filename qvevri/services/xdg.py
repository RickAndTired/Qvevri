...
class XDGGame(ServiceGame):
    ...
    def create_config(self):
        """Create a Qvevri config for the current game"""
        config = QvevriConfig(runner_slug=self.runner, game_config_id=self.config_id)
...
class XDGSyncer:
    """Sync games available in a XDG compliant menu to Qvevri"""
    ...
    ignored_executables = ("qvevri", "steam")
    ...
    def qvevri_games(self):
        """Iterates through Qvevri games imported from XDG"""
    ...
    def is_importable(cls, app):
        """Returns whether a XDG game is importable to Qvevri"""
        appid = get_appid(app)
        ...
        if any([
                ...
                appid.startswith("net.qvevri"),  # Skip qvevri created shortcuts
    ...
    def sync(self, games, full=False):
        """Sync the given games to the qvevri library

        Params:
            games (list): List of ServiceGames to sync
            full (bool): Run a full sync, removes games not found from the qvevri library

        Return:
            tuple: 2-tuple of added and removed game ID lists
        """
        installed_games = {game["slug"]: game for game in self.qvevri_games}
        available_games = set()
        added_games = []
        removed_games = []
        ...
        for slug in set(installed_games.keys()).difference(available_games):
            ...
            service_game = XDGGame.new_from_qvevri_id(game_id)
