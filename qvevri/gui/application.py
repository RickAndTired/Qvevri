...
from qvevri.gui.dialogs import ErrorDialog, InstallOrPlayDialog
from qvevri.gui.dialogs.issue import IssueReportWindow
from qvevri.gui.installerwindow import InstallerWindow
from qvevri.gui.widgets.status_icon import QvevriStatusIcon
from qvevri.migrations import migrate
from qvevri.command import exec_command
...
from qvevri.util import log
from qvevri.util.jobs import AsyncCall
from qvevri.util.log import logger
from qvevri.util.http import Request, HTTPError
from qvevri.api import parse_installer_url
from qvevri.startup import init_qvevri, run_all_checks
from qvevri.util.wine.dxvk import init_dxvk_versions, wait_for_dxvk_init

from .qvevriwindow import QvevriWindow


class Application(Gtk.Application):
    def __init__(self):
        super().__init__(
            application_id="net.qvevri.Qvevri",
            flags=Gio.ApplicationFlags.HANDLES_COMMAND_LINE,
        )
        init_qvevri()
        gettext.bindtextdomain("qvevri", "/usr/share/locale")
        gettext.textdomain("qvevri")

        GLib.set_application_name(_("Qvevri"))
        ...
        if os.geteuid() == 0:
            ErrorDialog(
                "Running Qvevri as root is not recommended and may cause unexpected issues"
            )

        try:
            self.css_provider.load_from_path(
                os.path.join(datapath.get(), "ui", "qvevri.css")
            )
        ...
        if hasattr(self, "add_main_option"):
            ...
            ErrorDialog(
                "Your Linux distribution is too old. Qvevri won't function properly."
            )

    def add_arguments(self):
        if hasattr(self, "set_option_context_summary"):
            self.set_option_context_summary(
                "Run a game directly by adding the parameter qvevri:rungame/game-identifier.\n"
                "If several games share the same identifier you can use the numerical ID "
                "(displayed when running qvevri --list-games) and add "
                "qvevri:rungameid/numerical-id.\n"
                "To install a game, add qvevri:install/game-identifier."
            )
        else:
            logger.warning(
        ...
        self.add_main_option(
            ...
            _("Print the version of Qvevri and exit"), None,
        )
        self.add_main_option(
        ...
        self.add_main_option(
            ...
            _("Execute a program with the qvevri runtime"), None,
        )
        self.add_main_option(
    ...
    def do_activate(self):
        if not self.window:
            self.window = QvevriWindow(application=self)
    ...
    def do_command_line(self, command_line):
        ...
        logger.info("Running Qvevri %s", settings.VERSION)
        migrate()
        run_all_checks()
        ...
        # Execute command in Qvevri context
        ...
        try:
            ...
            installer_info = self.get_qvevri_action(url)
        except ValueError:
    ...
    def execute_command(command):
        """Execute an arbitrary command in a Qvevri context
    ...
    def do_shutdown(self):
        logger.info("Shutting down Qvevri")
    ...
    def set_tray_icon(self):
        ...
        if active and not self.tray:
            self.tray = QvevriStatusIcon(application=self)
        if self.tray:
