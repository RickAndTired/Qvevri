"""Main window for the Qvevri interface."""
...
from qvevri import api, pga, settings
from qvevri.game import Game
from qvevri.game_actions import GameActions
from qvevri.sync import sync_from_remote
from qvevri.gui.installerwindow import InstallerWindow
from qvevri.runtime import RuntimeUpdater

from qvevri.util.log import logger
from qvevri.util.jobs import AsyncCall

from qvevri.util import http
from qvevri.util import datapath

# from qvevri.util.steam.watcher import SteamWatcher

from qvevri.services import get_services_synced_at_startup, steam

from qvevri.vendor.gi_composites import GtkTemplate

from qvevri.gui import dialogs
from qvevri.gui.widgets.sidebar import SidebarListBox
from qvevri.gui.widgets.services import SyncServiceWindow
from qvevri.gui.dialogs.runners import RunnersDialog
from qvevri.gui.config.add_game import AddGameDialog
from qvevri.gui.config.system import SystemConfigDialog
from qvevri.gui.views.list import GameListView
from qvevri.gui.views.grid import GameGridView
from qvevri.gui.views.menu import ContextualMenu
from qvevri.gui.views.store import GameStore
from qvevri.gui.views.game_panel import GamePanel, GenericPanel
from qvevri.gui.widgets.utils import IMAGE_SIZES, open_uri


@GtkTemplate(ui=os.path.join(datapath.get(), "ui", "qvevri-window.ui"))
class QvevriWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, application, **kwargs):
        ...
        qvevri_icon = Gtk.Image.new_from_icon_name("qvevri", Gtk.IconSize.MENU)
        qvevri_icon.set_margin_right(3)
        self.website_search_toggle.set_image(qvevri_icon)
        self.website_search_toggle.set_label("Search Qvevri.net")
        self.website_search_toggle.set_tooltip_text("Search Qvevri.net")
...
#        self.panel_revealer.set_reveal_child(self.right_side_panel_visible)
    ...
    def _init_actions(self):
        ...
        actions = {
            "browse-games": Action(lambda *x: open_uri("https://qvevri.net/games/")),
            "register-account": Action(
                lambda *x: open_uri("https://qvevri.net/user/register/")
            ),
            ...
            "open-forums": Action(lambda *x: open_uri("https://forums.qvevri.net/")),
            "open-discord": Action(lambda *x: open_uri("https://discord.gg/Pnt5CuY")),
            "donate": Action(lambda *x: open_uri("https://qvevri.net/donate")),
        }
    ...
    def sync_services(self):
        """Sync local qvevri library with current Steam games and desktop games"""
    ...
    def on_disconnect(self, *_args):
        ...
        dlg = dialogs.QuestionDialog(
            {"question": "Do you want to log out from Qvevri?", "title": "Log out?", }
        )
    ...
    def toggle_connection(self, is_connected, username=None):
        ...
        if is_connected:
            ...
            logger.info("Connected to qvevri.net as %s", username)
    ...
    def on_website_search_toggle_toggled(self, toggle_button):
        ...
        if toggle_button.props.active:
            ...
            self.search_entry.set_placeholder_text("Search Qvevri.net")
    ...
    def show_invalid_credential_warning(self):
        dialogs.ErrorDialog(
            "Could not connect to your Qvevri account. Please sign in again."
        )

    def show_library_sync_error(self):
        dialogs.ErrorDialog(
            "Failed to retrieve game library. "
            "There might be some problems contacting qvevri.net"
        )
