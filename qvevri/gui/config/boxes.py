...
from qvevri import settings, sysoptions
from qvevri.gui.widgets.common import VBox, Label, FileChooserEntry, EditableGrid
from qvevri.runners import import_runner, InvalidRunner
...
class ConfigBox(VBox):
    ...
    def generate_widgets(self, config_section):
        ...
        if config_section == "game":
            self.config = self.qvevri_config.game_config
            self.raw_config = self.qvevri_config.raw_game_config
        elif config_section == "runner":
            self.config = self.qvevri_config.runner_config
            self.raw_config = self.qvevri_config.raw_runner_config
        elif config_section == "system":
            self.config = self.qvevri_config.system_config
            self.raw_config = self.qvevri_config.raw_system_config
    ...
    def generate_file_chooser(self, option, path=None):
        ...
        if "default_path" in option:
            ...
            default_path = self.qvevri_config.system_config.get(config_key)
    ...
    def on_reset_button_clicked(self, btn, option, widget, wrapper):
        ...
        self.qvevri_config.update_cascaded_config()
...
class GameBox(ConfigBox):
    def __init__(self, qvevri_config, game):
        ConfigBox.__init__(self, game)
        self.qvevri_config = qvevri_config
        if game.runner_name:
            if not game.runner:
                try:
...
class RunnerBox(ConfigBox):
    ...
    def __init__(self, qvevri_config, game=None):
        ConfigBox.__init__(self, game)
        self.qvevri_config = qvevri_config
        try:
            runner = import_runner(self.qvevri_config.runner_slug)()
        except InvalidRunner:
            runner = None
        if runner:
            self.options = runner.get_runner_options()

        if qvevri_config.level == "game":
...
class SystemBox(ConfigBox):
    def __init__(self, qvevri_config):
        ConfigBox.__init__(self)
        self.qvevri_config = qvevri_config
        runner_slug = self.qvevri_config.runner_slug
        ...
        if qvevri_config.game_config_id and runner_slug:
            self.generate_top_info_box(
                "If modified, these options supersede the same options from "
