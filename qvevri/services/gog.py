...
from qvevri.services import AuthenticationError, UnavailableGame
from qvevri.util.http import Request, HTTPError
from qvevri.util import system
from qvevri.util.log import logger
from qvevri.util.resources import download_media
from qvevri.gui.dialogs import WebConnectDialog
from qvevri.services.service_game import ServiceGame
from qvevri.services.base import OnlineService
...
class GOGSyncer:
    ...
    def sync(cls, games, full=False):
        """Import GOG games to the Qvevri library"""
        gog_ids = [game.appid for game in games]
        if not gog_ids:
            return ([], [])
        qvevri_games = api.get_api_games(gog_ids, query_type="gogid")
        added_games = []
        for game in qvevri_games:
            game_data = {
                "name": game["name"],
                "slug": game["slug"],
                "year": game["year"],
