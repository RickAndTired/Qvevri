...
from qvevri.settings import CACHE_DIR
...
def get_xdg_basename(game_slug, game_id, base_dir=None):
    ...
    if base_dir:
        ...
        for path in [
                ...
                "net.qvevri.{}-{}.desktop".format(game_slug, game_id),
        ]:
            if system.path_exists(os.path.join(base_dir, path)):
                return path

    return "net.qvevri.{}-{}.desktop".format(game_slug, game_id)
...
def create_launcher(game_slug, game_id, game_name, desktop=False, menu=False):
    ...
    launcher_content = dedent(
        ...
        Name={}
        Icon={}
        Exec=qvevri qvevri:rungameid/{}
        Categories=Game
        """.format(
            game_name, "qvevri_{}".format(game_slug), game_id
        )
    )
