...
from qvevri import runtime, settings
from qvevri.gui.dialogs import DontShowAgainDialog, ErrorDialog
from qvevri.util import system
from qvevri.util.log import logger
from qvevri.util.strings import version_sort, parse_version
from qvevri.runners.steam import steam
...
def use_qvevri_runtime(wine_path, force_disable=False):
    """Returns whether to use the Qvevri runtime.
    ...
    if WINE_DIR in wine_path:
        logger.debug("%s is provided by Qvevri, using runtime", wine_path)
        return True
    if is_installed_systemwide():
        logger.info("Using system wine version, not using runtime")
        return False
    logger.debug("Using Qvevri runtime for wine")
    return True


def is_installed_systemwide():
    """Return whether Wine is installed outside of Qvevri"""
...
def is_version_esync(path):
    ...
    esync_compatible_versions = ["esync", "qvevri", "tkg", "ge", "proton"]
...
def display_vulkan_error(on_launch):
    ...
    DontShowAgainDialog(
        ...
        "<a href='https://github.com/qvevri/qvevri/wiki/How-to:-DXVK'>"
        "How-to:-DXVK (https://github.com/qvevri/qvevri/wiki/How-to:-DXVK)</a>",
...
def esync_display_limit_warning():
    ErrorDialog(
        ...
        " <a href='https://github.com/qvevri/qvevri/wiki/How-to:-Esync'>"
        "How-to:-Esync (https://github.com/qvevri/qvevri/wiki/How-to:-Esync)</a>"
    )
