...
from qvevri.util.yaml import read_yaml_from_file, write_yaml_to_file
...
class QvevriConfig:
    ...
    Description
    ===========
    Qvevri' configuration uses a cascading mechanism where
    ...
    def __repr__(self):
        return "QvevriConfig(level=%s, game_config_id=%s, runner=%s)" % (
            self.level,
            self.game_config_id,
            self.runner_slug,
        )
